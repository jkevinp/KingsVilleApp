  ...
  <div class="col-md-6"> 
    <div class="showback">
      <div class="btn-group btn-group-justified">
        <div class="btn-group">
          <a href="{{route('User.meter.reading.create')}}" class="btn btn-theme"><i class="fa fa-plus"></i> Create New Meter Reading</a>
        </div>
      </div>
    </div>
  </div>
    ...
    <thead>                       
      ...
      <tbody>
         @foreach($obj as $t)
          <tr>
              <td>{{$t['id']}}</td>
              <td><a href="{{route('User.account.profile' , $t['user_id'])}}">{{$t['meter_id']}}</a></td>
               <td>{{$t['lastreading']}}</td>
                <td>{{$t['currentreading']}}</td>
              <td>{{$t['readingdate']}}</td>
              <td>{{$t['details']}}</td>
              <td>
